    ...
    <section id="projects" className="py-16 sm:py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <div className={`relative transition-all duration-500 ease-out ${(headerVisible || isInitialLoad)
            ? 'opacity-100 translate-y-0'
                ...
                >
                  {filters.map((filter) => (
                    <button
                      ...
                      className={`relative px-3 py-3 rounded-xl text-sm font-medium transition-all duration-500 ease-out whitespace-nowrap ${activeFilter === filter.key
                        ? 'text-white'
                ...
                <div className="flex gap-2 overflow-x-auto w-full max-w-md">
                  {filters.map((filter) => (
                    <button
                      key={filter.key}
                      onClick={() => setActiveFilter(filter.key as ProjectFilter)}
                      className={`flex-shrink-0 px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${activeFilter === filter.key
        ...
        <div
          ref={contentRef}
          className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 transition-all duration-700 ease-out relative z-2 ${(contentVisible || isInitialLoad)
            ? 'opacity-100 translate-y-0'
            : 'opacity-0 translate-y-4'
            }`}
        >
          {filteredProjects.map((project, index) => (
            <div
              key={project.id}
              className={`group relative bg-background/80 backdrop-blur-sm rounded-2xl border-2 border-border/40 hover:border-accent/50 transition-all duration-300 ease-out overflow-hidden hover:shadow-lg hover:shadow-accent/10 ${(contentVisible || isInitialLoad)
                ? 'opacity-100 translate-y-0'
                : 'opacity-0 translate-y-4'
                }`}
                ...
                <div className="absolute top-4 left-4 space-y-2">
                  <span className="px-3 py-1.5 bg-accent text-white text-xs font-medium rounded-lg shadow-lg shadow-accent/25">
                    {project.category === 'wordpress' ? 'WordPress' :
                      project.category === 'native' ? 'Web App' :
                        project.category === 'testing' ? 'Testing' : 'UI Design'
                    }
                  </span>
                  {project.company && (
                        ...
                        <div className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-black sm:bg-accent/15 text-white sm:text-accent rounded-full border border-accent/30 backdrop-blur-sm">
                          ...
                          <span className="font-semibold text-xs sm:text-sm">
                            {selectedProject.category === 'wordpress' ? 'WordPress' :
                              selectedProject.category === 'native' ? 'Web App' :
                                selectedProject.category === 'testing' ? 'Testing' : 'UI Design'}
                          </span>
                        </div>
                      ...
                      <div className="flex justify-center gap-4 mt-4">
                        {selectedProject.images.map((_, index) => (
                          <button
                            ...
                            className={`w-3 h-3 rounded-full transition-all duration-300 hover:scale-125 ${index === currentImageIndex
